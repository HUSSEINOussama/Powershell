...
"chemin,Nom,Taille,Taille en MB,dn" | out-file "Homedir2Remove_$Args.csv"

$Folder = Get-ChildItem \\$Args.$Domain\Utilisateurs | where {$_ -notlike "*lotusid*"} | Where-Object{$_.PsISContainer}

#$cred= Get-Credential
#$conn=Connect-QADService -service "infocle.com" -credential $cred

		$conn=Connect-QADService -service "infocle.com" 
...
Foreach($InfoFolder In $Folder){
	...
	if (!$user){
		...
		"$Path,$Name,$Size,$SizeMB" | out-file "Homedir2Remove_$Args.csv" -append
	} 
}
